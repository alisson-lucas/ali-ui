export default function Features10() {
    return (
        <div className='w-full flex flex-row items-center justify-center flex-wrap bg-white p-10 lg:p-20' >
            <div className="flex flex-col w-full container items-center justify-center gap-10 lg:gap-20">
                <div className="relative flex flex-row flex-wrap w-full min-h-[900px] lg:min-h-[50px] items-center justify-center gap-10">
                    <div className="absolute -top-40 lg:-top-56 w-full flex flex-row flex-wrap items-center justify-center gap-10">
                        <article className="flex flex-col items-center justify-center w-full lg:w-[calc(100%/4-30px)] gap-10 py-10 p-4 bg-[#888888] hover:bg-[#04CD19] rounded-3xl">
                            <div className="flex flex-col items-start justify-center w-full 2xl:px-10 gap-8">
                                <span className="text-xl text-white font-semibold">Ensino otimizado</span>
                                <p className="text-white">Oferecer cursos de graduação, pós-graduação e extensão na área de teologia, com base na fé reformada e na confessionalidade congregacional.</p>
                            </div>
                        </article>
                        <article className="flex flex-col items-center justify-center w-full lg:w-[calc(100%/4-30px)] gap-10 py-10 p-4 bg-[#888888] hover:bg-[#04CD19] rounded-3xl">
                            <div className="flex flex-col items-start justify-center w-full 2xl:px-10 gap-8">
                                <span className="text-xl text-white font-semibold">Ensino otimizado</span>
                                <p className="text-white">Oferecer cursos de graduação, pós-graduação e extensão na área de teologia, com base na fé reformada e na confessionalidade congregacional.</p>
                            </div>
                        </article>
                        <article className="flex flex-col items-center justify-center w-full lg:w-[calc(100%/4-30px)] gap-10 py-10 p-4 bg-[#888888] hover:bg-[#04CD19] rounded-3xl">
                            <div className="flex flex-col items-start justify-center w-full 2xl:px-10 gap-8">
                                <span className="text-xl text-white font-semibold">Ensino otimizado</span>
                                <p className="text-white">Oferecer cursos de graduação, pós-graduação e extensão na área de teologia, com base na fé reformada e na confessionalidade congregacional.</p>
                            </div>
                        </article>
                        <article className="flex flex-col items-center justify-center w-full lg:w-[calc(100%/4-30px)] gap-10 py-10 p-4 bg-[#888888] hover:bg-[#04CD19] rounded-3xl">
                            <div className="flex flex-col items-start justify-center w-full 2xl:px-10 gap-8">
                                <span className="text-xl text-white font-semibold">Ensino otimizado</span>
                                <p className="text-white">Oferecer cursos de graduação, pós-graduação e extensão na área de teologia, com base na fé reformada e na confessionalidade congregacional.</p>
                            </div>
                        </article>
                    </div>
                </div>
            </div>
        </div>

    )
}